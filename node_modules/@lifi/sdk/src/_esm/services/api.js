...
export const getChains = async (params, options) => {
    ...
    const response = await request(`${config.get().apiUrl}/chains?${new URLSearchParams(params)}`, {
        signal: options?.signal,
    });
...
export const getTokens = async (params, options) => {
    ...
    return await request(`${config.get().apiUrl}/tokens?${new URLSearchParams(params)}`, {
        signal: options?.signal,
    });
};
/**
